...
| 参数      | 类型      | 默认值 | 说明                            |
| --------- | --------- | ------ | ------------------------------- |
| `id`      | `int`     | `-1`   | 字段 tag ID，或内部无 ID 哨兵值 |
...
- 配置的 ID 必须满足 `0 <= id < 2^29`（`0` 至 `536870911`）
- ID 必须在完整结构体 Schema 内唯一，包括继承字段
- 未指定时，注解默认值 `-1` 是内部无 ID 哨兵值，元数据中使用字段名（开销更大）
...
- [跨语言序列化](basic-serialization.md#cross-language-interoperability) - 与 Python、Rust、C++、Go 互操作
