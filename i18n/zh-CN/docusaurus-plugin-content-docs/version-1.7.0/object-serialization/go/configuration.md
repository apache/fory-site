...
所有根输入形式的默认限制固定为 `128 MiB`。正值会覆盖默认值；创建 Fory 实例时会拒绝显式设置的非正值。对象图内存预留是字节可用性检查的补充，而不是替代。未计入的叶值仍受剩余输入字节限制：如果未读输入没有足够字节，Fory 不会读取或创建该叶值。
