...
支持 `JsonValue` 字段和有效的 public 无参数方法，包括匹配的单 String `JsonCreator` 构造函数和 public static factory。
固定的 `JsonRawValue` 字段和 getter 支持可信的原始 String 值；`JsonByteArray` 字段和 getter 与 JVM 上一样
选择 Base64 字符串、Base16 字符串或数字字节数组。builder 的 `byteArrayFormat` 控制默认表示；
使用生成的对象编解码器时，应在 provider 中包含该设置。`escapeNonAscii(true)` 对字符串值和名称的作用
与 JVM 相同，使用生成的对象编解码器时也应在 provider 中配置。原始 JSON 保持原样。
`JsonFormat` 日期/时间字段使用与 JVM 相同的直接字段、单层包装和 `timezone` 行为。
对于直接放在目标类上的注解，请使用 `JsonType` 标注每个可达的所属模型，
以便 Native Image 保留这些成员和所选字节数组编解码器的构造函数。
