...
**当前状态**：Fory 1.0 是第一个稳定主版本线。请把主版本作为兼容性边界，并让生产端和消费端使用兼容的协议版本。

**建议**：

- 对跨语言载荷和运行时共享的 Schema 使用 xlang 模式
- 当运行时支持且流量只在同一语言内流转时，使用 native 模式
- Java Schema 独立变更时，使用 [Schema 演进](../guide/java/schema-evolution.md)
