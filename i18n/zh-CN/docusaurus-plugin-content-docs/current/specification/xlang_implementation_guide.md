...
头未命中时进入元数据解析路径：证明并读取元数据体字节，依据头验证数据体，验证字段数量，
并通过现有注册和反序列化策略检查解析类型。若解析前尚无本地头，此仅在未命中时执行的路径可以
延迟构建本地元数据，将其 52 位头哈希与已验证的接收哈希比较。哈希相等时选择本地元数据，
无需比较字节或字段，也不消耗远程 Schema 版本计数。
否则，构建并验证所需的读取状态。若 Schema 版本与逻辑类型缓存配额仍有空间，则发布到持久元数据缓存，
再记录 Schema 计数。缓存已满不会拒绝有效元数据；继续解码，但不缓存新 Schema 及其派生的序列化器和布局。
既有元数据引用在所需生命周期内持有这些未缓存状态。根重置必须防止旧逻辑查找和跨请求累计增长；
可复用底层槽位可以保留有界值直到被覆盖，无需立即物理清空。
失败或不兼容的元数据不得发布到持久缓存，也不得消耗 Schema 版本计数。
纯基于 ID 的 enum、ext 和 typed-union 值不携带 TypeDef 或 TypeMeta 数据体，
必须保持常规 type-id 加 user-type-id 路径。兼容的具名 enum、ext 和 union 元数据通常只有一个版本，
但在非本地元数据未命中后发布时，仍计入已缓存的远程元数据总数。`maxTypeFields` 仅适用于 struct 字段列表。
...
在 Java 中，头哈希标识编码 Schema，但针对同一个 Schema，请求的目标类可能需要不同的 `TypeInfo`。
仅以哈希为键的元数据缓存和深度提示保留源 `TypeInfo`。现有目标转换缓存按目标类、源类和头哈希
保留转换结果。只有元数据缓存或目标转换缓存未命中时才选择本地 Schema；之后命中时直接复用
选定结果，不再查询本地 TypeDef 元数据。
